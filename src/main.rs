...
struct ServerHTMLTemplate<'a> { // the name of the struct can be anything
    title: &'a str, // the field name should match the variable name
}

async fn index_html_handler() -> impl IntoResponse {
    let server_html = ServerHTMLTemplate { title: "Websocket Test Client" };
    HtmlTemplate(server_html)
}
